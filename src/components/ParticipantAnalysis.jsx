...
export default function SkillTrends() {
  ...
  const data = useMemo(() => {
    const baseData =
      skillTrends.length === 12
        ? skillTrends.map((d) => d.value)
        : [42, 48, 55, 58, 54, 50, 62, 78, 65, 52, 48, 70];
  ...
  }, [skillTrends, appliedFilters]);

  const growthStats = useMemo(() => {
    if (!appliedFilters) {
      return { growth: 12.4, label: "Stable Growth", badge: "Baseline" };
    }
  ...
  return (
    <div className={styles.page}>
      <div className={styles.layout}>
        <div className={styles.left}>
          ...
          <div className={styles.card}>
            <div className={styles.header}>
              <div>
                <p className={styles.subTitle}>GLOBAL PARTICIPANT SKILL TRENDS</p>
                <h2>Growth +{growthStats.growth}%</h2>
              </div>
              <span className={styles.badge}>{growthStats.badge}</span>
            </div>
          ...
          <div className={styles.participantsSection}>
            <div className={styles.participantsHeader}>
              <h2>Participant Insight Cards</h2>
              <span className={styles.count}>{filteredParticipants.length} Total</span>
            </div>

            <div className={styles.grid}>
              {filteredParticipants.map((p) => (
                <div className={styles.participantCard} key={p._id}>
                  <input
                    type="checkbox"
                  ...
                  <div className={styles.topRow}>
                    <div className={styles.avatar} style={{ backgroundColor: p.color }}>
                      {p.initials}
                    </div>
                    <div className={styles.info}>
                      <h3>{p.name}</h3>
                  ...
                  <div className={styles.metricsRow}>
                    <div className={styles.metricBox}>
                      <p className={styles.metricLabel}>PRIMARY SKILL</p>
                      <div className={styles.circleWrap}>
                        <svg className={styles.circle} viewBox="0 0 36 36">
                          <path
                            className={styles.bg}
                        ...
                        <span className={styles.circleText}>{p.skillPercent}%</span>
                      </div>
                      <p className={styles.skillName}>{p.skill}</p>
                    </div>
                      ...
                      <div className={styles.compatibilityBars}>
                        {[...Array(5)].map((_, i) => (
                          <div
                            key={i}
                            className={`${styles.vBar} ${
                              i < Math.round(p.compatibility / 20) ? styles.vBarActive : ""
                            }`}
                          />
                        ))}
                      </div>
                      <p className={styles.compatibilityScore}>{p.compatibility}% Score</p>
                    </div>
                  </div>
                </div>
              ))}
            ...
            {filteredParticipants.length === 0 && (
              <p style={{ padding: "16px", color: "#6b7280" }}>No participants match your search.</p>
            )}
          </div>
        </div>
        ...
        <div className={styles.sidebar}>
          ...
          {selected.map((p) => (
            <div key={p._id} className={styles.compareItem}>
              <div className={styles.compareAvatar}>{p.initials}</div>
              <div>
                <strong>{p.name}</strong>
                <p>{p.skill}</p>
          ...
          {selected.length > 0 && (
            <div className={styles.analysisCard}>
              <p className={styles.analysisTitle}>PERFORMANCE MATRIX</p>
              <div className={styles.radarWrap}>
                <Radar data={radarData} options={radarOptions} />
              </div>
            </div>
      ...
      {/* Advanced Filters Slide Panel */}
      <div className={`${styles.advancedOverlay} ${showAdvanced ? styles.open : ""}`}>
        <AdvancedFilters
          onApply={(filters) => {
